 ...
 * This file contains implementation for st_issimple.
 */
...
namespace sql
{
ObExprSTIsSimple::ObExprSTIsSimple(ObIAllocator &alloc)
    : ObFuncExprOperator(alloc, T_FUN_SYS_ST_ISSIMPLE, N_ST_ISSIMPLE, 1, VALID_FOR_GENERATED_COL, NOT_ROW_DIMENSION)
{
}

int ObExprSTIsSimple::calc_result_type1(ObExprResType &type,
                                       ObExprResType &type1,
...
int ObExprSTIsSimple::eval_st_issimple(const ObExpr &expr, ObEvalCtx &ctx, ObDatum &res)
{
  int ret = OB_SUCCESS;
  ...
  if (OB_FAIL(expr.args_[0]->eval(ctx, datum))) {
    ...
    } else if (OB_FAIL(ObGeoExprUtils::get_srs_item(ctx, srs_guard, wkb, srs, true, N_ST_ISSIMPLE))) {
      LOG_WARN("fail to get srs item", K(ret), K(wkb));
    } else if (OB_FAIL(ObGeoExprUtils::build_geometry(tmp_allocator, wkb, geo, srs, N_ST_ISSIMPLE, 
...
int ObExprSTIsSimple::cg_expr(ObExprCGCtx &expr_cg_ctx, const ObRawExpr &raw_expr, ObExpr &rt_expr) const
{
  UNUSEDx(expr_cg_ctx, raw_expr);
  rt_expr.eval_func_ = eval_st_issimple;
  return OB_SUCCESS;
}
